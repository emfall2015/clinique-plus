...
--
-- Host: localhost    Database: clinique_v2
-- ------------------------------------------------------
...
--
-- Temporary view structure for view `nuit_a_traiter`
--

DROP TABLE IF EXISTS `nuit_a_traiter`;
/*!50001 DROP VIEW IF EXISTS `nuit_a_traiter`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `nuit_a_traiter` AS SELECT 
 1 AS `id_nuit`,
 1 AS `nom`,
 1 AS `prenom`,
 1 AS `date_nuit`,
 1 AS `type_etude`,
 1 AS `notes_techniques`*/;
...
INSERT INTO `utilisateur` VALUES (1,'azerty','operateur',1),(2,'azerty','medecin',8),(3,'azerty','operateur',2);
/*!40000 ALTER TABLE `utilisateur` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Temporary view structure for view `v_medecins_validateurs`
--

DROP TABLE IF EXISTS `v_medecins_validateurs`;
/*!50001 DROP VIEW IF EXISTS `v_medecins_validateurs`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_medecins_validateurs` AS SELECT 
 1 AS `id_personnel`,
 1 AS `nom`,
 1 AS `prenom`,
 1 AS `specialite`*/;
...
--
-- Temporary view structure for view `v_nuits_traitées`
--

DROP TABLE IF EXISTS `v_nuits_traitées`;
/*!50001 DROP VIEW IF EXISTS `v_nuits_traitées`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_nuits_traitées` AS SELECT 
 1 AS `id_patient`,
 1 AS `nom`,
 1 AS `prenom`,
 1 AS `id_nuit`,
 ...
 1 AS `nb_rera`,
 1 AS `nb_microeveils`,
 1 AS `duree_sommeil_min`,
 1 AS `duree_hypoxie_min`,
 1 AS `position_dominante`,
 1 AS `duree_apnee_moy_sec`,
 1 AS `duree_apnee_max_sec`,
 1 AS `decibels_max`,
 1 AS `decibels_moy`,
 1 AS `nb_ronflements_forts`,
 1 AS `commentaire_medical`,
 1 AS `date_validation`*/;
SET character_set_client = @saved_cs_client;

--
-- Dumping events for database 'clinique_v2'
--

--
-- Dumping routines for database 'clinique_v2'
--
/*!50003 DROP PROCEDURE IF EXISTS `insert_data_night` */;
...
commentaire_medical TEXT)
BEGIN
	INSERT INTO resultat_nuit (id_nuit,id_medecin_validateur, date_validation, iah, spo2_min, spo2_moy, spo2_mediane, nb_apnees, nb_hypopnees, nb_rera, nb_microeveils, duree_sommeil_min, duree_hypoxie_min, position_dominante, duree_apnee_moy_sec, duree_apnee_max_sec, decibels_max, decibels_moy, nb_ronflements_forts,commentaire_medical)  VALUES
    (nuit,
    id_medecin_validateur, 
    current_date(),
    (SELECT SUM(valeur)*60/duree_sommeil_min FROM (select COUNT(type_evenement) as valeur FROM evenement_respiratoire WHERE (type_evenement = 'apnée obstructive' OR type_evenement = 'apnée centrale') AND id_nuit = nuit UNION ALL SELECT COUNT(type_evenement) as valeur FROM evenement_respiratoire WHERE (type_evenement = 'hypopnée') AND id_nuit = nuit) AS iah),
    spo2_min,
    spo2_moy,
    spo2_mediane,
    (SELECT (COUNT(type_evenement)) as nb_apnees FROM evenement_respiratoire WHERE (type_evenement = 'apnée obstructive' OR type_evenement = 'apnée centrale') AND id_nuit = nuit),
    (SELECT (COUNT(type_evenement)) as nb_hypopnees FROM evenement_respiratoire WHERE (type_evenement = 'hypopnée') AND id_nuit = nuit),
    (SELECT (COUNT(type_evenement)) as nb_rera FROM evenement_respiratoire WHERE (type_evenement = 'RERA') AND id_nuit = nuit),
    (SELECT (COUNT(type_evenement)) as nb_microeveils FROM evenement_respiratoire WHERE id_nuit = nuit),
    duree_sommeil_min,
    duree_hypoxie_min,
    position_dominante,
    (SELECT avg(duree_sec) as duree_apnee_moy_sec from evenement_respiratoire where id_nuit = nuit),
    (SELECT max(duree_sec) as duree_apnee_max_sec from evenement_respiratoire where id_nuit = nuit),
    decibels_max,
    decibels_moy,
    nb_ronflements_forts,
    commentaire_medical)
	ON DUPLICATE KEY UPDATE
    id_nuit = VALUES(id_nuit),
...
--
-- Final view structure for view `nuit_a_traiter`
--
...
/*!50001 VIEW `nuit_a_traiter` AS select `nuit_etude`.`id_nuit` AS `id_nuit`,`p`.`nom` AS `nom`,`p`.`prenom` AS `prenom`,`nuit_etude`.`date_nuit` AS `date_nuit`,`nuit_etude`.`type_etude` AS `type_etude`,`nuit_etude`.`notes_techniques` AS `notes_techniques` from ((`nuit_etude` left join `resultat_nuit` `r` on((`nuit_etude`.`id_nuit` = `r`.`id_nuit`))) left join `patient` `p` on((`p`.`id_patient` = `nuit_etude`.`id_patient`))) where (`r`.`id_nuit` is null) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_medecins_validateurs`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_medecins_validateurs` AS select `medecin`.`id_personnel` AS `id_personnel`,`personnel`.`nom` AS `nom`,`personnel`.`prenom` AS `prenom`,`medecin`.`specialite` AS `specialite` from (`medecin` left join `personnel` on((`personnel`.`id_personnel` = `medecin`.`id_personnel`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_nuits_traitées`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_nuits_traitées` AS select `p`.`id_patient` AS `id_patient`,`p`.`nom` AS `nom`,`p`.`prenom` AS `prenom`,`n`.`id_nuit` AS `id_nuit`,`n`.`date_nuit` AS `date_nuit`,`n`.`type_etude` AS `type_etude`,`m`.`id_personnel` AS `id_medecin`,`per`.`nom` AS `nom_medecin`,`per`.`prenom` AS `prenom_medecin`,`r`.`iah` AS `iah`,`r`.`severite_iah` AS `severite_iah`,`r`.`spo2_min` AS `spo2_min`,`r`.`spo2_moy` AS `spo2_moy`,`r`.`spo2_mediane` AS `spo2_mediane`,`r`.`nb_apnees` AS `nb_apnees`,`r`.`nb_hypopnees` AS `nb_hypopnees`,`r`.`nb_rera` AS `nb_rera`,`r`.`nb_microeveils` AS `nb_microeveils`,`r`.`duree_sommeil_min` AS `duree_sommeil_min`,`r`.`duree_hypoxie_min` AS `duree_hypoxie_min`,`r`.`position_dominante` AS `position_dominante`,`r`.`duree_apnee_moy_sec` AS `duree_apnee_moy_sec`,`r`.`duree_apnee_max_sec` AS `duree_apnee_max_sec`,`r`.`decibels_max` AS `decibels_max`,`r`.`decibels_moy` AS `decibels_moy`,`r`.`nb_ronflements_forts` AS `nb_ronflements_forts`,`r`.`commentaire_medical` AS `commentaire_medical`,`r`.`date_validation` AS `date_validation` from ((((`resultat_nuit` `r` join `nuit_etude` `n` on((`n`.`id_nuit` = `r`.`id_nuit`))) join `patient` `p` on((`p`.`id_patient` = `n`.`id_patient`))) join `medecin` `m` on((`m`.`id_personnel` = `r`.`id_medecin_validateur`))) join `personnel` `per` on((`per`.`id_personnel` = `m`.`id_personnel`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-07-17 11:49:30
